...
void Board::initBoard(){
    ...
    for (int i = 0; i < N; ++i){
        board[i][2] = board[i][3] = board[i][4] = board[i][5] = board[i][6] = board[i][7] = Men::None;
            ...
            board[1][0] = board[7][0] = board[2][0] = board[8][0] = Men::BKnight;
            board[1][9] = board[7][9] = board[2][9] = board[8][9] = Men::WKnight;
...
bool Board::canManMove(const QPoint &from, const QPoint &to){
    ...
    else if (abs(board[from.x()][from.y()]) == 7)
        return canQueenMove(from, to);
    else if (abs(board[from.x()][from.y()]) == 8)
        return canKingMove(from, to);
    else return false;
}
...
bool Board::canAchive(QPoint to, bool isWhite){
    ...
    if (board[x][y + 1 *int(pawn)]  == pawn) points.append({x, y + 1*int(pawn)});
