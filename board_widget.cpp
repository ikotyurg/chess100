...
{  
    ...
    for (int i = 0; i < nCells; ++i) {
        QLabel *verticalLabel = new QLabel(vLiterals[i], this);
        verticalLabel->setGeometry((i + 1) * cellSize, cellSize + 1, cellSize, cellSize); // Смещение вниз
        verticalLabel->setAlignment(Qt::AlignCenter);

        // Установка цвета фона для ячеек с обозначениями
        verticalLabel->setStyleSheet("QLabel { background-color : #a93400; font-size: 24px; font-weight: bold; color: beige; }");
    }

    // Цвет для угловых ячеек
    QString cornerColor = "#a93400"; // Вы можете изменить этот цвет на свой вкус
